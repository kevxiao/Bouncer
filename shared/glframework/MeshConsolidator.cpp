...
MeshConsolidator::MeshConsolidator(
    ...
    for(const ObjFilePath & objFile : objFileList) {
	    ...
	    appendVector(m_vertexTexCoords, uvCoords);
...
//----------------------------------------------------------------------------------------
// Returns the starting memory location for vertex normal data.
const float * MeshConsolidator::getVertexTexCoordsPtr() const {
    return &(m_vertexTexCoords[0].x);
}
...
//----------------------------------------------------------------------------------------
// Returns the total number of bytes of all vertex normal data.
size_t MeshConsolidator::getNumVertexTexCoordsBytes() const {
	return m_vertexTexCoords.size() * sizeof(vec2);
}
